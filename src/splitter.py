...
def Splitter(data: pd.DataFrame, year_col: str = "year"):
    ...
    after. The minimum dataset length is 4 years: 3 for the first training
    fold, and 1 for validation.
    ...
    ------
    ValueError
        If fewer than 4 unique years are present in ``year_col``.
    ValueError
    ...
    >>> folds = Splitter(df)
    >>> folds["Fold_1"]
    {'train': [2011, 2012, 2013], 'val': [2014]}
    >>> folds["Fold_2"]
    {'train': [2011, 2012, 2013, 2014], 'val': [2015]}
    """
    ...
    if len_years < 4:
        raise ValueError(
            f"There are not enough years in this dataset. This function needs a minimum of 4 years, found {len_years}."
        )
